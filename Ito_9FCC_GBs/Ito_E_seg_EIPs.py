...
import numpy as np
from Ito_ASE_setup import get_pure_bulk, get_GB, get_e_list, get_lattice_parameter, get_cal_sites, get_bulk_number_of_atom, get_GB_number_of_atom
from Ito_ASE_setup import define_calc, set_optimizer, get_pot_list_EIPs, create_needed_dir

######################################################################################################################
                                    # Functions for calculations
######################################################################################################################

def bulk_energy_cal(potchoice, GBchoice, optimizer, fmax, steps): # Calculate the energy of pure bulk system.
    print("\n\nCalculating E_bulk....\n")
...
def bulk_sub_energy_cal(potchoice, GBchoice, optimizer, fmax, steps): # Calculate the energy of bulk system with one solute.
...
def gb_sub_energy_cal(potchoice, GBchoice, cal_sites, sub_e, y_GB_plane, optimizer, fmax, steps, savetraj): # Calculate the energy of pure GB system with one solute.
    ...
    for i in range(number_of_sites):
        ...
        if savetraj == 'Yes':
            opt.attach(traj.write, interval=50)
        opt.run(fmax=fmax, steps=steps)
...
def segregation_energy_cal(potchoice, GBchoice, optimizer, fmax, steps, savetraj=None):
    '''
    Segregation energy:
    ...
    E_bulk = bulk_energy_cal(potchoice, GBchoice, optimizer, fmax, steps)
    E_bulk_sub = bulk_sub_energy_cal(potchoice, GBchoice, optimizer, fmax, steps)
    E_gb = gb_energy_cal(potchoice, GBchoice, optimizer, fmax, steps)
    E_gb_sub, site_dist = gb_sub_energy_cal_4alle(potchoice, GBchoice, cal_sites, y_GB_plane, optimizer, fmax, steps, savetraj)
